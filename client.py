...
def recivedMsg():
    ...
    while True:
        ...
        if("player_type" in message):
            recvmesg = eval(message)
            player_type = recvmesg['player_type']
            playerTurn = recvmesg['turn']
            ...
            handleResetGame(message)
        
        if('player1Turn' in message and playerType == 'player1'):
            playerTurn = True
            rollButton = Button(gameWindow,text='ROLL DICE',fg=("chalkboard SE",15),bg="grey",command=rollDice,width=20,height=5)
            rollButton.place(x=screen_width/2-80,y=screen_height/2+250)
        elif('player2Turn' in message and playerType == 'player2'):
            playerTurn = True
            rollButton = Button(gameWindow,text='ROLL DICE',fg=("chalkboard SE,15"),bg="grey",command=rollDice,width=20,height=5)
            rollButton.place(x=screen_height/2-80,y=screen_height/2+260)
        
        if('player1Turn' in message or 'player2Turn' in message):
            diceChoices = ['⚀','⚁','⚂','⚃','⚄','⚅']
            diceVal = diceChoices.index(message[0])+1
...
def handleWin(message):
    ...
    resetButton.place(x=screen_width/2-80,y=screen_height-200)
